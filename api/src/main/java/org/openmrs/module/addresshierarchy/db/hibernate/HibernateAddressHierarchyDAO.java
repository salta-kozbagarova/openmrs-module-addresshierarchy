...
public class HibernateAddressHierarchyDAO implements AddressHierarchyDAO {
	...
	public void deleteNonHierarchicalAddressEntriesByLevel(AddressHierarchyLevel level) {
		Session session = getCurrentSession();
		if(level.getNonHierarchical()==true){
			for (AddressHierarchyEntry entry : getAddressHierarchyEntriesByLevel(level)) {
				session.delete(entry);
			}
		}
	}
	...
	public List<AddressHierarchyLevel> getNonHierarchicalAddressLevels() {
		Session session = getCurrentSession();
		Criteria criteria = session.createCriteria(AddressHierarchyLevel.class);
		criteria.add(Restrictions.eq("nonHierarchical", true));
		return criteria.list();
	}
	...
	public AddressHierarchyLevel getTopAddressHierarchyLevel() {
		...
		Criterion parent = Restrictions.isNull("parent");
		Criterion nonHierarchical = Restrictions.eq("nonHierarchical", false);
		LogicalExpression andExp = Restrictions.and(parent, nonHierarchical);
		criteria.add(andExp);
